...
if [[ $1 =~ ^[0-9]+$ ]]; then
  ...
  if [[ $1 =~ ^[A-Za-z]{1,2}$ ]]; then
    ELEMENT_INFO=$($PSQL "SELECT e.atomic_number, e.name, e.symbol, t.type, p.atomic_mass, p.melting_point_celsius, p.boiling_point_celsius FROM elements e JOIN properties p ON e.atomic_number = p.atomic_number JOIN types t ON p.type_id = t.type_id WHERE e.symbol = INITCAP('$1')")
  else
    # Argument is a name
